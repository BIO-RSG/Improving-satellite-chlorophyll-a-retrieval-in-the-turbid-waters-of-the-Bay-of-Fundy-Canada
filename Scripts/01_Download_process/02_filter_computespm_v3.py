...
def main(*args):
  ...
  chlor_a = chlor_a[ind] # WAIT TO DO THE IND LATER why??
  ...
  # CHL-A NOW #### 
  # Make sure rrs where SPM Nechad is NaN are also NaN
  rrs_443[np.isnan(spmnechad)] = nan
  rrs_488[np.isnan(spmnechad)] = nan
  rrs_547[np.isnan(spmnechad)] = nan
  
  # Also make sure done for chl layers that aren't originating from RRS here
  ...
  rrs_ratio_chl_488=np.log10(rrs_ratio_chl_488) #checked
  ...
  #0.24240, -2.74230,  1.80170,  0.00150, -1.22800
  ...
  print('Valid pixels: '+str(longikeep.size))
  # print(nb_good_pxl)
  ...
  if nb_good_pxl>0:
      ...
      out_data[:,1]=latikeep
      out_data[:,2]=spmhan # Here we store the new SPM in column 3 to generate grd spm
      out_data[:,3]=spmdox
