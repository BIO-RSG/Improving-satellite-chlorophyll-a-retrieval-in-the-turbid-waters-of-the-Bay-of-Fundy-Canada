# This script gets lists of L2 files by searching the NASA earthdata site in a bounding box and over a time period.
# The output is lists of file URLs to use with wget (02_download_L2.sh)
# LOOP 1: Scrape files in a bounding box and metadata (no coverage meta yet)
# LOOP 2: Filter files to daytime, sun above horizon, other time filts if wanted.
# Andrea Hilborn 2021
...
for (i in minyear:maxyear) {
  pagenum = 1
  # There are a limited number of results per page, so we loop through all pages available
  while(pagenum > 0) {
    ...
    if (nchar(data[[2]]) > 1) {
      # Save to file
      filename=paste0("01_l2_files/BF","_",i,"_",minlon,"_",maxlon,"_",minlat,"_",maxlat,"_page",pagenum,"_raw.csv")
      message(filename)
      write.csv(x = data, file = paste0("./scripts/MODISA/01_Download_process/",filename), quote = F, row.names = F)
      pagenum = pagenum+1
    } else {
      pagenum = 0
    }
...
yearfilelist <- list.files("./scripts/MODISA/01_Download_process/01_l2_files", "raw.csv", full.names = T)
for (i in minyear:maxyear) {
  ...
  # Calculating sun angle, remove if < 0
  ...
  # Filtering only to day files - might be redundant / not necessary after altitude filter
  ...
  write.table(img_urls, file = paste0("./scripts/MODISA/01_Download_process/01_l2_files/",filename), quote = F, row.names = F, col.names = F)
}
